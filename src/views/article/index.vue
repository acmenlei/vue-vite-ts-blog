...
<template>
    <article class="blog-card" v-for="(item, index) in articleData" :key="index">
        <img class="post-image" :src="item.ll_cover" />
        <div class="article-details">
            <div class="post-header-contain">
                <h4 class="post-category">{{ item.ll_category }}</h4>
                <div class="post-three-words">
                    <span v-for="word in 3"></span>
                </div>
            </div>
            <h2 class="post-title">{{ item.ll_title }}</h2>
            <p class="post-description">{{ item.ll_introduce }}</p>
